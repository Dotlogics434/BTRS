...
class Users_profile extends Access_user {
	...
	function get_language($user, $pw) {
		$sql = sprintf("SELECT up.language AS lang FROM %s AS u, %s AS up WHERE u.login = '%s' AND u.pw = '%s' AND u.id = up.users_id ", $this->table_name, $this->profile_tbl_name, $user, md5($pw));
